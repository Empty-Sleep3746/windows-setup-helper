@ECHO OFF
REM == Auto-detect ISO in script directory ==
for %%f in ("%~dp0*.iso") do (
    set "sourceiso=%%~f"
    goto :setmediapath
)
...
:setmediapath
for %%I in ("%sourceiso%") do (
    set "isoname=%%~nI"
    set "isoext=%%~xI"
)
set "mediapath=%~dp0%isoname%"
set "outputiso=%~dp0%isoname%-modified%isoext%"
echo %mediapath%
REM Directory of extra files to add to the image (no trailing slash)
set "extrafiles=%~dp0Additions"
...
set "wimindex=/Name:Microsoft Windows Setup (x64)"
set wimindex="/Index:1"
set wimindex="/Index:2"
set helperrepo=%~dp0
